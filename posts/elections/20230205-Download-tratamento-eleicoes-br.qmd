...
details_mun_zone_fed = electionsBR::details_mun_zone_fed(year)
vote_mun_zone_fed = electionsBR::vote_mun_zone_fed(year)
```
...
preciso for o padrão definido, menores serão os gastos com tempo e com memória RAM. 
...
### Abordagem de leitura para redução de gasto de memória RAM
...
RAM de sua máquina é carregar uma tabela de cada vez, tratar os dados dessa tabela 
...
Nesta seção, eu aplico as sugestões que propus no tópico anterior, operacionalizando 
...
Sendo que, a exclusão da última base orignal carregada na memória junto ao *reset* da sessão do R
gerou uma nova redução de consumo de memória de 932 mb. 
